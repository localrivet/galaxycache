...
import (
	...
	gc "github.com/localrivet/galaxycache"
	pb "github.com/localrivet/galaxycache/galaxycachepb"
...
func (g *grpcFetcher) Remove(ctx context.Context, galaxy string, key string) error {
	span := trace.FromContext(ctx)
	span.Annotatef(nil, "fetching from %s; connection state %s", g.address, g.conn.GetState())
	_, err := g.client.RemoveFromPeer(ctx, &pb.RemoveRequest{
		Galaxy: galaxy,
		Key:    key,
	})
	if err != nil {
		return status.Errorf(status.Code(err), "Failed to fetch from peer over RPC [%q, %q]: %s", galaxy, g.address, err)
	}
