...
func (gp *serviceImpl) RemoveFromPeer(ctx context.Context, req *pb.RemoveRequest) (*pb.RemoveResponse, error) {
	galaxy := gp.universe.GetGalaxy(req.Galaxy)
	if galaxy == nil {
		return nil, status.Errorf(codes.InvalidArgument, "Unable to find galaxy [%s]", req.Galaxy)
	}
	...
	err := galaxy.Remove(ctx, req.Key)
	if err != nil {
		return nil, status.Errorf(status.Code(err), "Failed to remove [%s]: %v", req, err)
	}
