...
import (
	...
	"github.com/localrivet/galaxycache/consistenthash"
)
...
func (pp *PeerPicker) GetAll() []RemoteFetcher {
	...
	var i int
	res := make([]RemoteFetcher, len(pp.fetchers))
	for _, v := range pp.fetchers {
		res[i] = v
		i++
	}
	return res
...
func (n *nullFetchFetcher) Remove(ctx context.Context, galaxy string, key string) error {
	return errors.New("empty remover")
}

// func (n *nullFetchFetcher) GetAll() []RemoteFetcher
